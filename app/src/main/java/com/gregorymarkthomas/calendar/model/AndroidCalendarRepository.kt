 ...
 */
class AndroidCalendarRepository(private val contentResolver: Resolver, private val permissionContract: AndroidPermissionContract): CalendarRepository {

    override fun getCalendars(callback: Callback.GetCalendarsCallback) {
        callback.onGetCalendars(AndroidCalendarRetriever(contentResolver, permissionContract).get())
    }
    ...
    override fun getEvents(fromDayInMonth: Int, month: Int, year: Int, specifiedNoOfDays: Int, callback: Callback.GetEventsCallback, calendarsToShow: IntArray) {
        callback.onGetEvents(AndroidCalendarEventResolver(contentResolver, permissionContract).get(fromDayInMonth, month, year, specifiedNoOfDays, calendarsToShow))
    }
}